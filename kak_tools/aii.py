...
def aii_kak(v, omega, verify=0):
    ...
    if verify:
        ...
        assert np.allclose(h @ omega, omega @ h.T, atol=1e-6), np.amax(
            np.abs(h @ omega - omega @ h.T)
        )  # omega skew-sym condition
        assert np.allclose(k.T @ omega @ k, omega, atol=1e-6), np.amax(
            np.abs(k.T @ omega @ k - omega)
        )  # symplectic condition
        assert np.allclose(w1.T @ omega @ w1, omega, atol=1e-6), np.amax(
            np.abs(w1.T @ omega @ w1 - omega)
        )  # symplectic condition

        #                       k        a       k   = v
        assert np.allclose(w1.conj().T @ d @ (w1 @ k), v, atol=1e-6), (
            w1.conj().T @ d @ (w1 @ k),
            v,
        )
