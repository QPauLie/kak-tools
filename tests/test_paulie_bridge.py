...
def expected_evolution(generators, coefficients, time, n):
    h = sum(c * w.to_mat(wire_order=range(n))
            for c, w in zip(coefficients, as_pauli_words(generators)))
...
def test_external_zero_coefficients_do_not_change_the_generator_family():
    with pytest.raises(ValueError, match="horizontal"):
        kak_decomposition(["X", "Y", "Z"], [1, 1, 0])
